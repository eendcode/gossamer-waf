...
func TestFirefox(t *testing.T) {
	...
	r.Header.Set("user-agent", "Mozilla/5.0 (Macintosh; Intel Mac OS X x.y; rv:42.0) Gecko/20100101 Firefox/42.0")

	b, err := browser.New(r)
	if err != nil {
		t.Errorf("error on parsing browser: %v", err)
	}

	if err := b.ComplianceCheck(); err != nil {
		t.Errorf("compliance error: %v", err)
	}
}
...
func TestChrome(t *testing.T) {
	...
	if !b.IsMobile {
		t.Error("incorrectly classified as non-mobile")
	}
...
func TestFaultyChrome(t *testing.T) {
	...
	if err := b.ComplianceCheck(); err == nil {
		t.Errorf("expected failure of compliancy check, but passed")
	}
